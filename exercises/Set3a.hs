...
firstHalf xs = take half xs
  where
    half = if even (length xs) then half' else half' + 1
...
capitalize = unwords . map capitalizes . words

capitalizes :: String -> String
capitalizes xs = toUpper (head xs) : tail xs
...
-- infix, partial application. instead of (\x -> x <= max)
...
joinToLength l xs = [c | a <- xs, b <- xs, let c = a ++ b, l == length c]
...
sumRights = sum . map toNum 
    where toNum x = case x of (Right a) -> a
                              (Left _) -> 0
...
multiCompose [] a =  a
multiCompose fs a = (head fs . multiCompose (tail fs)) a
