...
treeMax Empty = 0
treeMax (Node a l r) = if childMax > a then childMax else a
  where
    leftMax = treeMax l
    rightMax = treeMax r
    childMax = if leftMax > rightMax then leftMax else rightMax
...
allValues condition tree = case tree of
  Empty -> True
  (Node a l r) -> condition a && (applyCondition l && applyCondition r)
  where
    applyCondition = allValues condition
...
mapTree f t = case t of
  Empty -> Empty
  (Node a l r) -> Node (f a) (partialMap l) (partialMap r)
  where
    partialMap = mapTree f
...
cull val tree = case tree of
  Empty -> Empty
  (Node a l r) -> if a == val then Empty else Node a (cull val l) (cull val r)
...
--   where
--     inOrderMax Empty visitedMax = visitedMax
--     inOrderMax (Node a l r) visitedMax =
--       let ordered Empty = (< a) <$> visitedMax <|> Just True
--           ordered tree = (< a) <$> inOrderMax tree visitedMax
