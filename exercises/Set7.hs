...
velocity d t = case (d, t) of
  (Distance a, Time b) -> Velocity $ (/) a b
...
add a aSet = if a `member` aSet then aSet else case aSet of (Set as) -> Set $ sort (a : as)
...
data State = Start | Error | Finished | EggAdded | FlourAdded | SugarAdded | FlourSugarAdded | Mixed
  deriving (Eq, Show)
...
step state event = case (state, event) of
  ...
  (FlourAdded, AddSugar) -> FlourSugarAdded
  (SugarAdded, AddFlour) -> FlourSugarAdded
  (FlourSugarAdded, Mix) -> Mixed
  (Mixed, Bake) -> Finished
  (Finished, _) -> Finished
  (_, _) -> Error
...
average as = case as of
  (a :| []) -> a
  (a :| aas) -> Set4a.average $ a : aas

------------------------------------------------------------------------------
-- Ex 5: reverse a NonEmpty list.

reverseNonEmpty :: NonEmpty a -> NonEmpty a
reverseNonEmpty as = case as of
  (a :| []) -> a :| []
  (a :| as) ->
    let ras = nonEmpty (foldl (flip (:)) [] (a : as))
     in case ras of
          (Just nas) -> nas
          Nothing -> a :| []
...
instance Ord a => Semigroup (Set a) where
  (<>) (Set a) (Set []) = Set a
  (<>) (Set a) (Set b) = foldl (flip add) (Set b) a
...
passwordAllowed psd a = case a of
  (MinimumLength l) -> length psd >= l
  (ContainsSome pattern) -> foldl (\b pa -> b || elem pa pattern) False psd
  (DoesNotContain pattern) -> not $ passwordAllowed psd (ContainsSome pattern)
  (And req1 req2) -> passwordAllowed psd req1 && passwordAllowed psd req2
  (Or req1 req2) -> passwordAllowed psd req1 || passwordAllowed psd req2
...
render a = case a of
  (Literal a) -> show a
  (Multiply a b) -> renderOp "*" a b
  (Add a b) -> renderOp "+" a b
  where
    renderOp op a b = "(" ++ render a ++ op ++ render b ++ ")"
