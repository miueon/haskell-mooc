...
countHelper x ys = case x of Nothing -> ys + 1
                             _ -> ys
...
slStart = (0.0, 0)
slHelper x (sum, count) = (sum + x, count + 1) 
...
largestHelper x [] = [x]
largestHelper x (y:ys) = case compare x y of EQ -> x:y:ys
                                             GT -> [x]
                                             LT -> y:ys
...
lastHelper x m = case m of Nothing -> Just x
                           _ -> m
