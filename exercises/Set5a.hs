...
data Person = PersonUndefined | MkPerson {age::Int, name::String}
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = MkPerson 24 "senbai"

-- getName returns the name of the person
getName :: Person -> String
getName p = case p of PersonUndefined ->   ""
                      (MkPerson _ name) -> name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge p = case p of PersonUndefined -> 0
                     (MkPerson age _) -> age
...
data Position = PositionUndefined | Position {x::Int, y::Int}
...
study s = case s of Freshman -> NthYear 1
                    (NthYear n) -> if n == 7 then Graduated else NthYear (n + 1)
                    Graduated -> Graduated
...
get :: UpDown -> Int
get ud = case ud of (Increase a) -> a
                    (Decrease a) -> a

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick ud = case ud of (Increase a) -> Increase (a + 1)
                     (Decrease a) -> Decrease (a - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle ud = case ud of (Increase a) -> Decrease a
                       (Decrease a) -> Increase a
...
vectorPlus xs [] = xs
vectorPlus [] ys = ys
vectorPlus (x:xs) (y:ys) = (x + y) : vectorPlus xs ys
------------------------------------------------------------------------------
